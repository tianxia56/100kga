...
# Plot PCA results
def plot_pca(merged_df, variance_explained_percentage):
    unique_supop = merged_df['SUPOP'].unique()
    unique_pop = merged_df['POP'].unique()
    ...
    for pop in unique_pop:
        subset = merged_df[merged_df['POP'] == pop]
    ...
    plt.title('PCA of 100kGA 1168 samples')
    ...
    plt.tight_layout()
    plt.savefig('all.100kga_pca_plot.png')
    print("PCA plot saved as 'all.100kga_pca_plot.png'.")
...
def main():
    ...
    plot_pca(merged_df, variance_explained_percentage)
